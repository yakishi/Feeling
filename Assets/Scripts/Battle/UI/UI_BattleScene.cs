    ...
    void InputEvent()
    {
        ...
        if (Input.GetKeyDown(KeyCode.LeftArrow)) {
            ...
            if (myEnemyArrow.enemyArrowFlg && myEnemyArrow.enemyArrowFlgCnt != -1) { // 敵選択フラグが有効なとき
                ...
                //}
                int temp = myEnemyArrow.enemyArrowFlgCnt - 1;

                while (temp >= 0 && BattleEnemyGenerate.Enemy[temp] == null) {
                    --temp;
                }

                if (temp < 0) {
                    temp = myEnemyArrow.enemyArrowFlgCnt;
                }
                myEnemyArrow.enemyArrowFlgCnt = temp;
