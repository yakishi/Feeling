...
public class Attack /*: Combat !注意 : extends すると二回呼び出されてしまう！*/
{
    ...
    public void PlayerAttack(string choiceName)
    {
        ...
        for (int i = 0; i < myCombatState.Count; i++) {
            ...
            for (int j = 0; j < Combat.GetCombatState.Count; j++) {
                if (Combat.GetCombatState[j].isAction) {
                    Debug.Log(i + "番目が行動可能状態です。");
                    Combat.GetCombatState[j].isAction = false; // 攻撃コマンドが押されたら行動終了とする
                                                               // TODO 上の Sleep 関数のようにする
                    Combat.GetCombatState[j + 1].isAction = true; // 攻撃コマンドが押された時に次ぎの人に回す
